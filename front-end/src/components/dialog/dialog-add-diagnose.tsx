...
export const DialogAddDiagnose = ({ open, setOpen, select, title }: {
  open: boolean,
  ...
  const { isFetching } = useQuery({
    enabled: open,
    queryKey: ["form-diagnoses"],
    queryFn: async () => {

      const response = await httpClient.get<response<Diagnose>>(`/api/diagnoses/${select}`)
            ...
            <form action="#"
              ...
              <div className="space-y-3">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-sm font-medium">Solusi</FormLabel>
                      <FormControl>
