...
export default function RegisterPage() {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async (data: FormData) => {
      return await httpClient.post<response<User>>("/api/auth/register", data)
    },
    onSuccess: (response) => {
    ...
    onError: (error: any) => {
      if (error.response?.status === 422) {
        ...
        toast.error("Terjadi kesalahan", {
          description: error.response?.data?.message || error.message || "Gagal memproses permintaan",
        })
      }
    },
  })
                  ...
                  <div className="grid gap-3">
                    ...
                    <div className="space-y-2 mb-2">
                      <Button type="submit"
                              ...
                              disabled={isPending}>
                        <span className="group-data-[loading=true]:invisible">Submit</span>
                        <LoaderCircleIcon
